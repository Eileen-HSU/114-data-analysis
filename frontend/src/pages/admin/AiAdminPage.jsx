...
const statusText = (value) => ({ validated: "Ready to publish", needs_validation: "Pending validation", published: "Live", not_published: "Not published" }[value] || value);
...
export default function AiAdminPage() {
  ...
  if (!canAccess) return <><Navbar /><main className="ai-admin-empty"><h1>AI administration is restricted to administrators</h1><button onClick={() => navigate("/workspace")}>Back to Analysis Assistant</button></main></>;
  return <><Navbar /><main className="ai-admin-page">
    <header><p className="eyebrow">INTERNAL ADMINISTRATION</p><h1>AI classification administration</h1><p>Maintain classification rules, monitor quality, and manage system settings.</p></header>
    ...
    {tab === "manage" && <section>
      {!selected ? <div className="topic-grid">{topics.map((topic) => <article key={topic.prompt_key} className="topic-card"><h2>{topic.prompt_key}</h2><p>Live configuration: <b>{statusText(topic.production_status)}</b></p><p>Candidate：<b>{statusText(topic.candidate_status)}</b></p><small>Last updated: {topic.updated_at ? new Date(topic.updated_at).toLocaleString("en-US") : "—"}</small><div><button onClick={() => openTopic(topic.prompt_key)}>View settings</button><button className="primary" onClick={() => openTopic(topic.prompt_key)}>Continue editing</button></div></article>)}</div> : <section className="candidate-workspace"><button className="back" onClick={() => setSelected(null)}>← All classification topics</button><h2>{selected} Candidate workspace</h2><div className="stepper">{["edit", "test", "publish"].map((item, i) => <button key={item} className={step === item ? "active" : ""} onClick={() => setStep(item)} disabled={item === "publish" && !(validation?.can_publish || detail?.draft_validated)}>{i + 1}. {item === "edit" ? "Edit" : item === "test" ? "Test" : "Publish"}</button>)}</div>
      {step === "edit" && <div className="admin-card"><details><summary>View live configuration (read only)</summary><pre>{detail?.live_content}</pre></details><label>Candidate content<textarea value={draft} onChange={(e) => setDraft(e.target.value)} rows="16" /></label><button className="primary" onClick={save}>Save changes (validation results will be invalidated)</button></div>}
      {step === "test" && <div className="admin-card"><h3>Sandbox quick test</h3><textarea value={sandboxText} onChange={(e) => setSandboxText(e.target.value)} placeholder="Enter an open-ended response" rows="4" /><button onClick={sandbox}>Run (without changing live data)</button>{sandboxResult && <pre className="result">{JSON.stringify(sandboxResult, null, 2)}</pre>}<hr /><h3>Validation: Golden Test Cases</h3><button className="primary" onClick={validate}>Run all validation cases</button>{validation && <><p className={validation.can_publish ? "pass" : "fail"}>{validation.can_publish ? "Ready to publish" : "Not ready to publish"} · Valid format  {validation.format_valid_count}/{validation.total} · Golden accuracy  {(validation.accuracy_vs_golden * 100).toFixed(0)}%</p><div className="case-list">{validation.details.map((item, i) => <p key={i} className={item.is_correct ? "pass" : "fail"}>{item.is_correct ? "✓" : "×"} Expected: {item.expected_sub_category}/ Result: {item.actual_sub_category}</p>)}</div></>}</div>}
      {step === "publish" && <div className="admin-card"><h3>Publish candidate</h3><p>The validated candidate will become the live configuration.</p><button className="primary" disabled={!(validation?.can_publish || detail?.draft_validated)} onClick={publish}>Publish as live configuration</button></div>}</section>}</section>}
    {tab === "reviews" && <section className="admin-card"><div className="filter-row"><label>Classification topic  <select value={reviewTopic} onChange={(e) => { setReviewTopic(e.target.value); loadClassifications(reviewStatus, e.target.value); }}><option value="">All</option>{topics.map(x => <option key={x.prompt_key} value={x.prompt_key}>{x.prompt_key}</option>)}</select></label><label>review_status <select value={reviewStatus} onChange={(e) => { setReviewStatus(e.target.value); loadClassifications(e.target.value, reviewTopic); }}><option value="">All</option>{["confirmed", "modified", "pending_review", "excluded"].map(x => <option key={x}>{x}</option>)}</select></label></div>{classifications.map((row) => <article className="review-row" key={row.classification_id}><p><b>{row.review_status}</b> · {row.segment}</p><p>Effective：{row.effective_result.main_category} / {row.effective_result.sub_category}</p>{row.review_status === "modified" && <details><summary>Compare AI Original and Human Review Final</summary>{["main_category", "sub_category", "secondary_sub_category", "reasoning"].filter(k => row[k] !== row[`final_${k}`]).map(k => <p key={k}><b>{k}</b><br />AI：{row[k] || "—"}<br />Human：{row[`final_${k}`] || "—"}</p>)}<button onClick={() => navigator.clipboard.writeText(`Response: ${row.segment}\nAI：${JSON.stringify({main_category: row.main_category, sub_category: row.sub_category, reasoning: row.reasoning})}\nHuman：${JSON.stringify(row.effective_result)}`)}>Copy mismatched cases</button></details>}</article>)}</section>}
    {tab === "system" && <section className="admin-card"><div className="subtabs"><button className={systemTab === "taxonomy" ? "active" : ""} onClick={() => setSystemTab("taxonomy")}>Classification schema</button><button className={systemTab === "golden" ? "active" : ""} onClick={() => setSystemTab("golden")}>Golden Test Cases</button></div>{systemTab === "taxonomy" ? systemData.map(topic => <details key={topic.prompt_key}><summary>{topic.prompt_key}</summary>{topic.subcategories.map(item => <article key={item.sub_category}><b>{item.main_category} / {item.sub_category}</b><p>Methodology：{item.methodology}</p><p>Citation：{item.citation}</p></article>)}</details>) : <div className="case-list">{systemData.map((item, i) => <article key={i}><b>{item.prompt_key}</b><p>Input: {item.answer_text}</p><p>Expected：{item.expected_sub_category}</p></article>)}</div>}</section>}
  </main></>;
}
