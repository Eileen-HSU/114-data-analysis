...
const Navbar: React.FC<NavbarProps> = ({ onLoginRequired, scrolled = true, transparent = false }) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 w-full px-8 py-0 flex items-center justify-between z-50 h-[72px] transition-all duration-300 ${
      isTransparent
      ...
      >
        <div className={`w-10 h-10 flex items-center justify-center rounded-xl transition-all ${
          isTransparent
            ...
            : "bg-gradient-to-br from-violet-500 via-sky-500 to-cyan-500 group-hover:from-violet-400 group-hover:to-cyan-400"
        }`}>
          <i className="ri-bar-chart-box-line text-white text-xl"></i>
        </div>
        <div className="flex flex-col leading-none">
          <span className={`text-xl font-black tracking-tight transition-colors ${isTransparent ? "text-white" : "text-slate-800"}`}>DataAnalysis</span>
          <span className={`text-[11px] tracking-widest uppercase font-semibold transition-colors ${isTransparent ? "text-white/70" : "bg-gradient-to-r from-violet-500 to-sky-500 bg-clip-text text-transparent"}`}>智能資料分析平台</span>
        </div>
      </Link>
      ...
      <div className="flex items-center gap-3 flex-1 justify-end">
        {isLoggedIn && user ? (
          ...
          <div className="flex items-center gap-3">
            <Link
              to="/login"
              className={`px-5 py-2.5 text-base font-semibold transition-colors whitespace-nowrap cursor-pointer ${
                isTransparent ? "text-white/90 hover:text-white" : "text-slate-600 hover:text-violet-600"
              }`}
            >
              登入
            </Link>
            <Link
              to="/signup"
              className={`px-5 py-2.5 text-base font-bold rounded-xl transition-all whitespace-nowrap cursor-pointer ${
                isTransparent
                  ? "bg-white/25 text-white border border-white/40 hover:bg-white/35"
                  : "bg-gradient-to-r from-violet-500 via-sky-500 to-cyan-500 text-white hover:from-violet-400 hover:to-cyan-400"
              }`}
            >
              免費註冊
            </Link>
