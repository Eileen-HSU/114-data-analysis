"""Run with (from the backend/ directory): python -m unittest tests.test_workspace_sharing -v

改放進 tests/ 資料夾後，這支必須用「-m unittest 模組路徑」的方式執行
（而不是直接 python3 tests/test_workspace_sharing.py），因為它沒有另外
用 sys.path.insert 補路徑，是靠 `-m` 會把目前工作目錄（backend/）加進
sys.path 來解析 extensions / models / routes 這些頂層模組。
"""
import os
import sys
import unittest
...
# 保險起見仍加上這行：即使哪天改用別的方式執行，也能正確解析到
# backend/ 底下的頂層模組（models、extensions、routes...）。
